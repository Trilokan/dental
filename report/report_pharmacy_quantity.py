...
class ReportPharmacyQuantity(models.TransientModel):
    _name = "report.pharmacy.quantity"
    ...
    detail_ids = fields.One2many(comodel_name="report.pharmacy.quantity.detail", inverse_name="report_id")
    ...
    def trigger_report(self):
        ...
        if self.person_id:
            recs = self.env["invoice.detail"].search([("invoice_id.date", ">=", self.from_date),
                                                      ...
                                                      ("invoice_id.invoice_type", "=", "pharmacy"),
...
class ReportPharmacyQuantityDetail(models.TransientModel):
    _name = "report.pharmacy.quantity.detail"
    ...
    report_id = fields.Many2one(comodel_name="report.pharmacy.quantity", string="Pharmacy")
